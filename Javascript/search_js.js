...
var tp = document.getElementById('search_class');

    var li= tp.getElementsByTagName('li');
...
const makeAllBackgrounds = () => {
    Array.from(document.getElementsByClassName('gridtry')).forEach((element) => {
...
Array.from(document.getElementsByClassName('playListPlay')).forEach((element) => {
    element.addEventListener('click', (e) => {
        ...
        makeAllBackgrounds();
        Array.from(document.getElementsByClassName('gridtry'))[`${index - 1}`].style.background = "rgb(105, 105, 170, .1)";
    })
})
...
back.addEventListener('click', () => {
    index -= 1;
    if (index < 1) {
        index = Array.from(document.getElementsByClassName('gridtry')).length;
    }
    music.src = `Songs/${index}.mp3`;
    poster_master_play.src = `i/${index}.jfif`;
    music.play();
...
next.addEventListener('click', () => {
    index -= 0;
    index += 1;
    if (index > Array.from(document.getElementsByClassName('gridtry')).length) {
        index = 1;
    }
    music.src = `Songs/${index}.mp3`;
    poster_master_play.src = `i/${index}.jfif`;
